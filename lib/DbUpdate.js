...
exports.insert = function (props, res) {
    ...
    if (client.query('BEGIN', err => {
        client.query(sql, values, (err, res) => {
            if (err) {
                res.status(500).json({err: 'startgame.js could not write to db.'});
                console.error('insert to db failed.', err.stack);
            } else {
                gameId = res.rows[0].id;
                client.query('COMMIT', err => {
                if (err) {
                    res.status(500).json({err: 'insert commit failed.'});
                    console.error('insert commit failed', err.stack);
                } else {
                    client.end((err, res) => {
                        if (err) {
                            res.status(500).json({err: 'error during db disconnect.'});
                            console.error('error during db disconnect.', err.stack);
                        } else {
                            // everything is GOOD.
