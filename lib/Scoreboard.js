...
// constructor
function Scoreboard(type, cb1, cb2) {
    if (type === "PR") {
        game = new PrGame();
    } else if (type === "OLR") {
        game = new OlrGame();
...
/**
 * Deregister (remove) the player from the playerList.
 */
...
/**
 * Register (add) player for the round. Implementation differs as per the game in progress.
 */
Scoreboard.prototype.registerPlayer = function (name) {
    let accepted = false;
    if (game.getType() === "CR") {
        // for Countdown Round see if new name is expected (should be on playerList). If not then reject;
        // otherwise update the list and turn name black as visual indicator to moderator.
        for (let i = 0; i < playerList.length; i++) {
...
Scoreboard.prototype.recordBuzz = function (name, answer) {
    playerList.forEach(function(e,i,a) {
        ...
        console.log("buzzOrder: %d numberIncorrect: %d", buzzOrder, numberIncorrect);
                if (buzzOrder === 1) {
                    playerList[i].selected = true;
