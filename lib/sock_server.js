...
logger.debug(`raw answer: ${rawAnswer}`);
        ...
        Promise.all(qaPromises).then( (results) => {
            ...
            admin_channel.emit('show-answer', answer);
            projector_channel.emit('ask-question', {item: question, size: question.length >= LONG_LENGTH? 4:3});
	...
	socket.on('show-answer', function(data) {
		...
		projector_channel.emit('show-answer', {item: data, size: 1});
		logger.info('  show-answer>>all admin/projectors');
	});
